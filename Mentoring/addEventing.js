...
/* Solution (satisfies most requirements) */

function addEventing(obj) {

  const eventsMap = {
    // need to keep track of which callbacks belong to which events
  }
  // code here

  function on(event, callback) {
    const eventToTrack = { [event] : callback }
    Object.assign(eventsMap, eventToTrack)
  }


  function trigger(event) {
    const cb = eventsMap[event]
    cb()
  }

  return {on, trigger, ...obj}
}
...
obj.trigger('click') // logs 'clicked!'
